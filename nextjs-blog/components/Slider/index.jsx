  ...
  useEffect(() => {
    if (slides && slides?.length > 0) {
      const interval = setInterval(() => {
        setCurrentIndex(
          (prevIndex) => (prevIndex + 1) % (slides ? slides?.length : 0)
        );
      }, 4000);
      ...
      <div className="slider w-full h-full overflow-hidden">
        {slides.map((slide, index) => (
          <Link
            // href={slide.addr}
            href={
              router?.asPath.includes("#search_products")
                ? router?.asPath
                : router?.asPath + "#search_products"
            }
            //  target="_blank"
          >
            <div
              key={slide.seq}
              className={`slide w-full h-full transition-opacity duration-500 ease-in-out ${
            ...
            >
              <div className="relative w-full md:h-64 h-40">
                ...
                <div className="backdrop-blur absolute md:top-80 top-48 left-0 right-0 bottom-0 flex items-center justify-center backdrop-blur backdrop-filter backdrop-saturate-150">
                  <h1 className="font-serif font-bold md:text-6xl text-4xl font-bold text-sky-600">
                    {slide.name}
                  </h1>
                </div>
              </div>
